...
import { 
  ...
  Printer, BookOpen, Star, Trophy, HeartHandshake, X
} from "lucide-react";
import { 
      ...
      <DialogContent className="w-[94vw] max-w-md max-h-[90vh] overflow-y-auto p-0 border-2 border-amber-500/40 shadow-2xl rounded-2xl sm:rounded-3xl focus:outline-none [&>button.absolute]:hidden">
        ...
        <div className={`p-5 sm:p-6 text-center text-white relative overflow-hidden ${
          ...
          <Badge className="bg-white/20 backdrop-blur-md text-white border border-white/30 text-[9px] sm:text-[10px] font-black uppercase tracking-wider mb-2">
            Rotational Winner · {cycle.cycleLabel}
          </Badge>
          ...
          <div className="text-center space-y-1">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2 pt-1.5 sm:pt-2">
              <div className="p-2 rounded-xl bg-muted/60 text-center">
                ...
                <span className="text-xs sm:text-sm font-black text-primary">{winner.points}</span>
              </div>
              <div className="p-2 rounded-xl bg-muted/60 text-center">
                <span className="text-[9px] sm:text-[10px] text-muted-foreground block font-semibold">Books Issued</span>
                <span className="text-xs sm:text-sm font-black text-foreground">{winner.booksIssuedCount}</span>
              </div>
              <div className="p-2 rounded-xl bg-muted/60 text-center">
                <span className="text-[9px] sm:text-[10px] text-muted-foreground block font-semibold">Total Score</span>
                <span className="text-xs sm:text-sm font-black text-emerald-600">{winner.compositeScore}</span>
              </div>
            </div>
          </div>

          {/* Collection Date Callout Box (Crucial User Requirement) */}
          <div className="rounded-xl border-2 border-primary/30 bg-primary/5 p-3 sm:p-3.5 space-y-2 sm:space-y-2.5 shadow-2xs">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm font-black text-primary leading-tight mt-0.5 break-words">
            ...
            {cycle.settings.librarianNote && (
              <div className="pt-1 text-[10px] sm:text-[11px] text-muted-foreground italic border-t border-primary/15 break-words">
