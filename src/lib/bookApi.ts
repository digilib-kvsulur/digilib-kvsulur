 ...
 * from Google Books API and falls back to Open Library API.
 */
...
const CATEGORY_KEYWORDS = [
  ...
  "Poetry", "Drama", "Philosophy", "Religion", "Technology", "Textbook", "Reference"
];
    ...
    if (olRes.ok) {
      ...
      if (data[key]) {
        ...
        return {
          ...
          language: info.publish_places?.[0]?.name ? "English" : "", // Open Library data is sparse for language
        };
      }
    }
  } catch (error) {
